...
NONRIGID_ALIGNMENT_PARAMETER_MAPS = ["translation", "rigid", "bspline"]
SUPPORTED_TRANSFORM_MODELS = ("rigid", "bspline")


def _parameter_maps_for_transform_model(transform_model: str) -> list[str]:
    if transform_model == "rigid":
        return list(DEFAULT_ALIGNMENT_PARAMETER_MAPS)
    if transform_model == "bspline":
        return list(NONRIGID_ALIGNMENT_PARAMETER_MAPS)
    raise ValueError(
        "transform_model must be one of: "
        + ", ".join(SUPPORTED_TRANSFORM_MODELS)
    )
...
class AlignmentResult:
    ...
    Attributes:
        output_image_path: Final aligned image path, or ``None`` for dry runs.
        temp_dir: Retained temporary directory path when ``keep_temp_dir=True``; otherwise ``None``.
        output_transform_json_path: Transform metadata sidecar path, when requested.
        transform_metadata: In-memory transform metadata, when requested.
        dry_run: Whether the raster output was intentionally skipped.
    """
...
def _sample_bilinear_grid_extent(
    data: np.ndarray,
    row_coords: np.ndarray,
    col_coords: np.ndarray,
    *,
    ...
    """Sample values whose pixels cover the full grid extent, not just centers."""
    height, width = data.shape
    valid = (
        (row_coords >= -0.5)
        & (row_coords <= height - 0.5)
        & (col_coords >= -0.5)
        & (col_coords <= width - 0.5)
    )
    clamped_rows = np.clip(row_coords, 0.0, height - 1.0)
    clamped_cols = np.clip(col_coords, 0.0, width - 1.0)
    sampled, _ = _sample_bilinear(
        data,
        clamped_rows,
        clamped_cols,
...
def _apply_parameter_map_to_points(parameter_map: Any, points_xy: np.ndarray) -> np.ndarray:
    ...
    raise ValueError(f"Unsupported elastix transform kind for JSON export: {transform_kind}")


def _apply_parameter_object_to_points(transform_parameter_object: Any, points_xy: np.ndarray) -> np.ndarray:
    ...
    for idx in range(int(transform_parameter_object.GetNumberOfParameterMaps())):
        transformed = _apply_parameter_map_to_points(
            transform_parameter_object.GetParameterMap(idx),
            transformed,
        )
    return transformed


def _single_pass_target_to_source_matrix(
    *,
    ...
    bounds = rasterio.windows.bounds(
        Window(0, 0, int(fixed_window.width), int(fixed_window.height)),
        fixed_window_transform,
    )
    center_x = 0.5 * (bounds[0] + bounds[2])
    center_y = 0.5 * (bounds[1] + bounds[3])
    step = max(
        float(np.hypot(solve_transform.a, solve_transform.d)),
        float(np.hypot(solve_transform.b, solve_transform.e)),
        1.0,
    )
    target_xy = np.array(
        [
    ...
    source_solve_xy = _apply_parameter_object_to_points(
        transform_parameter_object,
        np.column_stack([solve_cols, solve_rows]),
    )
    ...
    return fit_affine_matrix(target_xy, np.column_stack([source_x, source_y]))
...
def align_image_pair(
    ...
    *,
    ...
    Args:
        ...
        output_image_path: Path for the coregistered output raster. Required unless ``dry_run=True``.
        ...
        transform_model: Registration transform model. ``"rigid"`` is the
            default global translation/rotation model. ``"bspline"`` is an
            experimental nonrigid model that supports only single-pass raster
            output; dry-run transform JSON, saved-transform application, vector
            QA, chunked solves, and multi-pass solve resolutions are not
            available for this mode.
        output_transform_json_path: Optional path for a JSON sidecar describing
            the final coordinate transform. Required when ``dry_run=True``.
        dry_run: If ``True``, estimate the transform and write/return transform
            metadata without writing an aligned raster.
    ...
    """
    parameter_maps = _parameter_maps_for_transform_model(transform_model)
    if transform_model != "rigid":
        if dry_run:
            raise ValueError("dry_run is not supported when transform_model='bspline'.")
        if output_transform_json_path is not None:
            raise ValueError(
                "output_transform_json_path is not supported when "
                "transform_model='bspline' because nonrigid transforms are not "
                "represented by Coregix matrix JSON."
            )
        if split_factor > 0:
            raise ValueError("split_factor must be 0 when transform_model='bspline'.")
    ...
    if solve_resolutions is not None:
        ...
        if transform_model != "rigid" and len(solve_resolutions) > 1:
            raise ValueError(
                "multi-pass solve_resolutions are not supported when "
    ...
    with rasterio.open(fixed_image_path) as fixed_src, rasterio.open(moving_image_path) as moving_src:
        ...
        if output_transform_json_path is not None:
            ...
            metadata["transform_model"] = "elastix_translation_rigid_world_affine"
            metadata["elastix_parameter_maps"] = parameter_object_to_list(transform_parameter_object)
            add_matrix_metadata(
                metadata,
                _single_pass_target_to_source_matrix(
        ...
        if dry_run:
            ...
            with rasterio.open(temp_output_image_path, "w+", **out_profile) as out_dst:
                ...
                for b in range(1, moving_src.count + 1):
                    ...
                    if output_on_moving_grid:
                        target_width = int(moving_window.width)
                        target_height = int(moving_window.height)
                        target_transform = moving_window_transform
                    else:
                        target_width = int(fixed_window.width)
                        target_height = int(fixed_window.height)
                        target_transform = fixed_window_transform
                    for row_off in range(0, target_height, int(block_height)):
                        for col_off in range(0, target_width, int(block_width)):
                            win_w = min(int(block_width), target_width - col_off)
                            win_h = min(int(block_height), target_height - row_off)
                            ...
                            dx_block, dx_valid = _sample_bilinear_grid_extent(
                                fixed_dx,
                                solve_rows,
                                solve_cols,
                                fill_value=0.0,
                            )
                            dy_block, dy_valid = _sample_bilinear_grid_extent(
                                fixed_dy,
                                solve_rows,
                                solve_cols,
                                fill_value=0.0,
